...
from app.models.enums import UserRole, OperationType, OperationStatus, PfiStatus, VoucherStatus, VoucherCategory
...
from app.schemas.pfi import PfiCreate, PfiGenerateRequest, PaymentCreate, StandalonePfiCreate, PfiConfirmPaymentRequest
...
class PfiService:
    ...
    async def generate_pfi(
        ...
        # Transition to pfi_linked only when the operation is in the correct
        # pre-PFI state for its type. If already pfi_linked or further along,
        # just record the PFI without re-transitioning.
        _PRE_PFI_STATE = {
            OperationType.truck_only:       OperationStatus.active,
            OperationType.full_operation:   OperationStatus.bdn_approved,
            OperationType.vessel_only:      OperationStatus.bdn_approved,
        }
        _POST_PFI_STATES = {
            OperationStatus.pfi_linked, OperationStatus.payment_processing,
            OperationStatus.payment_confirmed, OperationStatus.invoiced,
            OperationStatus.completed, OperationStatus.archived,
        }
        expected_pre = _PRE_PFI_STATE.get(operation.type)
        if expected_pre and operation.status == expected_pre:
            await _transition_operation(
                operation, OperationStatus.pfi_linked, current_user, db,
            ...
            # Inform the BM which state is needed
            needed = expected_pre.value if expected_pre else "the appropriate state"
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail=(
                    f"Cannot generate PFI: operation is in '{operation.status.value}'. "
                    f"It must reach '{needed}' before a PFI can be generated and linked."
                ),
            )
